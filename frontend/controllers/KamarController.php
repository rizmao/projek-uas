...
class KamarController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($this->request->isPost) {
            ...
            if ($model->file_upload) {
                ...
                $model->file_upload->saveAs('@frontend/web/img/' . $fileName);
    ...
    {
        if (($model = Kamar::findOne(['id' => $id])) !== null) {
            return $model;
        }
    ...
    public function actionFilter()
    {
        ...
        $rows = $db->createCommand("SELECT * FROM kamar WHERE category_id = :category_id")
                    ->bindValue(':category_id', $_GET['category_id'])
                    ->queryAll();
        $categories = KamarCategory::find()->all();
